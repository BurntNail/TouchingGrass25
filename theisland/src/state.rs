...
impl IslandState {
    pub async fn new () -> Result<Self, IslandError> {
        ...
        let bucket = {
            ...
            let creds = Credentials::new(Some(&access_key_id), Some(&secret_access_key), None, None, None)?;
            let region = Region::Custom {region, endpoint};
            Bucket::new(&bucket_name, region, creds)?
        };

        Ok(Self {
            update_leaderboard, redis_connection, bucket
        })
    }
    ...
    async fn get_top_info (conn: &mut MultiplexedConnection) -> Result<Vec<InternalTopImage>, IslandError> {
    ...
    pub async fn get_top_images (&self) -> Result<Vec<TopImageEntry>, IslandError> {
        ...
        for info in current_top {
            let image = self.bucket.presign_get(
                info.s3_path,
                60,
                None
            ).await?;
    ...
    pub async fn set_potential_top_image (&self, person: String, image_score: u32, image: Vec<u8>) -> Result<(), IslandError> {
        let mut redis_conn = self.redis_connection.clone();
        ...
        current_top.insert(index_to_insert, InternalTopImage {
            person,
            image_score,
            s3_path,
        });
